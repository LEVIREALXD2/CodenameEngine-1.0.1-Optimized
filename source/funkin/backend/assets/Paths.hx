...
class Paths
{
	...
	static public function image(key:String, ?library:String, checkForAtlas:Bool = false, ?ext:OneOfTwo<Array<String>, String>)
	{
		final exts:Array<String> = ext == null ? Flags.IMAGE_EXTS : Std.isOfType(ext, Array) ? cast ext : [cast ext];
		if (checkForAtlas)
		{
			for (e in exts)
			{
				final atlasPath = getPath('images/$key/spritemap.$e', library);
				if (atlasPath != null && OpenFlAssets.exists(atlasPath))
					return atlasPath.substr(0, atlasPath.length - 14);

				final multiplePath = getPath('images/$key/1.$e', library);
	...
	public static inline function astc(key:String) {
		return getPath('images/$key.astc');
	}

	public static inline function dds(key:String) {
		return getPath('images/$key.dds');
	}
	...
	/*inline*/ static public function getSparrowAtlasAlt(key:String, ?ext:OneOfTwo<Array<String>, String>)
	{
		final exts:Array<String> = ext == null ? Flags.IMAGE_EXTS : Std.isOfType(ext, Array) ? cast ext : [cast ext];
		for (e in exts)
			if (getPath('$key.$e') != null)
				return FlxAtlasFrames.fromSparrow('$key.$e', '$key.xml');
		return null;
	}
	...
	/*inline*/ static public function getPackerAtlasAlt(key:String, ?ext:OneOfTwo<Array<String>, String>)
	{
		final exts:Array<String> = ext == null ? Flags.IMAGE_EXTS : Std.isOfType(ext, Array) ? cast ext : [cast ext];
		for (e in exts)
			if (getPath('$key.$e') != null)
				return FlxAtlasFrames.fromSpriteSheetPacker('$key.$e', '$key.txt');
		return null;
	}
	...
	/*inline*/ static public function getAsepriteAtlasAlt(key:String, ?ext:OneOfTwo<Array<String>, String>)
	{
		final exts:Array<String> = ext == null ? Flags.IMAGE_EXTS : Std.isOfType(ext, Array) ? cast ext : [cast ext];
		for (e in exts)
			if (getPath('$key.$e') != null)
				return FlxAtlasFrames.fromAseprite('$key.$e', '$key.json');
		return null;
	}
	...
	**/
	 ...
	 */
	static function loadFrames(path:String, Unique:Bool = false, Key:String = null, SkipAtlasCheck:Bool = false, SkipMultiCheck:Bool = false, ?Ext:OneOfTwo<Array<String>, String> = null):FlxFramesCollection
	{
		var noExt = Path.withoutExtension(path);
		final exts:Array<String> = Ext == null ? Flags.IMAGE_EXTS : Std.isOfType(Ext, Array) ? cast Ext : [cast Ext];
